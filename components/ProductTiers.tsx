      ...
      <div className="max-w-[980px] mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          {t.productTiers.tiers.map((tier, idx) => (
            <div key={idx} className={`relative p-8 rounded-[28px] flex flex-col transition-all duration-300 hover:scale-[1.01] ${idx === 1 ? 'bg-[#ff5c5c]/8 border-2 border-[#ff5c5c]/30' : 'bg-[#161920] border border-[#1e2028]'}`}>
              {idx === 1 && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-[#ff5c5c] text-[#f4f4f5] px-4 py-1 rounded-full text-xs font-medium">
                  {t.productTiers.popularBadge}
                </div>
              )}
              <div className="mb-8">
                <h3 className="text-2xl font-semibold text-[#f4f4f5] mb-2 tracking-[-0.02em]">{tier.name}</h3>
                <div className="h-0.5 w-10 bg-[#ff5c5c] rounded-full"></div>
              </div>
              ...
              <div className="mb-8 flex-grow">
                ...
                <div className="p-4 bg-[#13151b] rounded-2xl border border-[#1e2028]">
                  <p className="text-xs font-medium text-[#ff5c5c] uppercase tracking-wider mb-2 flex items-center">
              ...
              <button className={`w-full py-3.5 rounded-full font-medium transition-all duration-300 text-[15px] ${idx === 1 ? 'bg-[#ff5c5c] hover:bg-[#ff7070] text-white font-semibold' : 'bg-[#191c24] hover:bg-[#1f2330] text-[#f4f4f5]'}`}>
